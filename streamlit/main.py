...
BACKEND_URL = os.getenv("BACKEND_URL", "http://tiny-llm-agent-backend-svc:8000")
...
if uploaded_file is not None:
    ...
    if st.button("📤 Upload and Process PDF"):
        with st.spinner("Processing your PDF ... ⏳"):
            ...
            response = requests.post(f"{BACKEND_URL}/api/upload_pdf?user_id={user_id}", files=files)
...
if prompt:
    ...
    with st.spinner("Thinking ... 💭"):
        response = requests.post(
            url=f"{BACKEND_URL}/api/chat?user_id={user_id}",
        ...
        if response.status_code == 200:
            ...
            # Hiển thị chi tiết lỗi backend trả về
